...
const totalcard= GAMES_JSON.reduce( (acc, card) => {
    return acc + card.pledged;
},0);
// set inner HTML using template literal
raisedCard.innerHTML = `$${totalcard.toLocaleString()}`;
...
const unfund= GAMES_JSON.filter(
  game => game.pledged < game.goal
);
const unfundedNum = unfund.length


// create a string that explains the number of unfunded games using the ternary operator

const displayStr = `We have at the moment ${unfundedNum} game${unfundedNum === 1 ? '' : 's'} that are unfunded.`
...
let [first, second]= sortedGames  //similar to challenge advice
